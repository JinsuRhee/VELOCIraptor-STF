...
Int_t RAMSES_get_nbodies(char *fname, int ptype, Options &opt)
{
    ...
    for (j = 0; j < NRAMSESTYPE; j++)
        ramses_header_info.npartTotal[j] = 0;
    ...
    */
    ...
    for (i = 0; i < ramses_header_info.num_files; i++)
    {
        ...
        for (int ii2 = 0; ii2 < ramses_header_info.npartlocal; ii2++)
            dummy_family[ii2] = dummy_family2[ii2] + 0;
        ...
        for (j = 0; j < ramses_header_info.npartlocal; j++)
        {
            ...
            if (dummy_family[j] == 3)
                ramses_header_info.npart[RAMSESSINKTYPE]++;
            // else
            //     if (dummy_age[j] != 0.0)
            //         ramses_header_info.npart[RAMSESSTARTYPE]++;
            //     else
    ...
    if (ptype == PSTALL || ptype == PSTDARK)
        opt.numpart[DARKTYPE] = ramses_header_info.npartTotal[RAMSESDMTYPE];
    if (ptype == PSTALL || ptype == PSTGAS)
        opt.numpart[GASTYPE] = ramses_header_info.npartTotal[RAMSESGASTYPE];
    if (ptype == PSTALL || ptype == PSTSTAR)
        opt.numpart[STARTYPE] = ramses_header_info.npartTotal[RAMSESSTARTYPE];
    if (ptype == PSTALL || ptype == PSTBH)
        opt.numpart[BHTYPE] = ramses_header_info.npartTotal[RAMSESSINKTYPE];
    return nbodies;
}
...
void ReadRamses(Options &opt, vector<Particle> &Part, const Int_t nbodies, Particle *&Pbaryons, Int_t nbaryons)
{
    ...
    if (opt.iBaryonSearch)
        mpi_nsend_baryon = new Int_t[NProcs * NProcs];
    if (opt.nsnapread > 1)
    {
        mpi_nsend_readthread = new Int_t[opt.nsnapread * opt.nsnapread];
        if (opt.iBaryonSearch)
            mpi_nsend_readthread_baryon = new Int_t[opt.nsnapread * opt.nsnapread];
    }
            ...
            for (i = 0; i < opt.num_files; i++)
            {
                if (ireadfile[i])
                {
                    ...
                    for (int nn = 0; nn < chunksize; nn++)
                        famtempchunk[nn] = famtempchunk2[nn] + 0;
                    // RAMSES_fortran_read(Fpartid[i],idvalchunk);
                    for (int nn = 0; nn < nchunk; nn++)
                    {
                        // if (fabs((mtempchunk[nn]-dmp_mass)/dmp_mass) > 1e-5 && (agetempchunk[nn] == 0.0))
                        // if (fabs((mtempchunk[nn]-dmp_mass)/dmp_mass) > 1e-5)
                        //{
                        //   GHOST PARTIRCLE!!!
                        //}
                        // else
...
//             for (int kk=0;kk<3;kk++) {xtemp[kk]=LittleRAMSESFLOAT(xtemp[kk]);vtemp[kk]=LittleRAMSESFLOAT(vtemp[kk]);}
                        ...
                        mtemp = 1.0;
                            ...
                            if (fabs((mtemp - dmp_mass) / dmp_mass) < 1e-5 && famtempchunk[nn] == 1)
                                typeval = DARKTYPE;
                            if (famtempchunk[nn] == 2)
                                typeval = STARTYPE;
                            if (famtempchunk[nn] == 3)
                                typeval = BHTYPE;
...
*/
        ...
        if (ireadtask[ThisTask] >= 0)
        {
            ...
            for (i = 0; i < opt.num_files; i++)
                if (ireadfile[i])
                {
                    ...
                    if (lmin > header[i].nlevelmax)
                        lmin = header[i].nlevelmax;
                    if (lmax < header[i].nlevelmax)
                        lmax = header[i].nlevelmax;
                    //@}
    ...
    if (opt.nsnapread > 1)
    {
        ...
        if (opt.iBaryonSearch)
            delete[] mpi_nsend_readthread_baryon;
        if (ireadtask[ThisTask] >= 0)
            delete[] Preadbuf;
    }
    delete[] Nbuf;
